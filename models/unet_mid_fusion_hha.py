...
class Encoder(nn.Module):
    def __init__(self, dropout=0.0):
        super().__init__()
        self.layer1 = resnet.make_layer(resnet.Bottleneck, 64, blocks=3, planes=16, stride=1)
        self.fusion_conv1 = nn.Conv2d(64, 256, kernel_size=1, stride=2)
        self.layer2 = resnet.make_layer(resnet.Bottleneck, 256, blocks=4, planes=64, stride=1)
        self.fusion_conv2 = nn.Conv2d(256, 512, kernel_size=1, stride=2)
        self.layer3 = resnet.make_layer(resnet.Bottleneck, 512, blocks=6, planes=128, stride=1)
        self.fusion_conv3 = nn.Conv2d(512, 1024, kernel_size=1, stride=2)
        self.layer4 = resnet.make_layer(resnet.Bottleneck, 1024, blocks=3, planes=256, stride=1)
        self.fusion_conv4 = nn.Conv2d(1024, 2048, kernel_size=1, stride=2)
        self.layer5 = resnet.make_layer(resnet.Bottleneck, 2048, blocks=3, planes=512, stride=1)
        self.dropout = nn.Dropout2d(p=dropout)
